      ...
      return (
        <div style={{ padding: '8px 20px 36px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <button onClick={() => applyRemap('week')} style={{ padding: '16px', borderRadius: 8, cursor: 'pointer', textAlign: 'left', background: 'var(--accent)11', border: '1px solid var(--accent)55' }}>
            ...
            <button onClick={() => applyRemap('meso')} style={{ padding: '16px', borderRadius: 8, cursor: 'pointer', textAlign: 'left', background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
          ...
          <button onClick={() => setConfirmScope(false)} style={{ width: '100%', marginTop: 12, padding: '12px', borderRadius: 8, background: 'transparent', border: 'none', color: 'var(--text-muted)', fontSize: 13, cursor: 'pointer' }}>← Back</button>
        </div>
      );
    }
      ...
      <div style={{ padding: '8px 20px 36px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 6, marginBottom: 20 }}>
          {DOW_FULL.map((name, dow) => {
            ...
            return (
              <button key={dow} onClick={() => toggleDay(dow)} style={{ padding: '10px 4px', borderRadius: 8, cursor: 'pointer', background: isSel ? 'var(--accent)22' : 'var(--bg-deep)', border: `1px solid ${isSel ? 'var(--accent)' : 'var(--border)'}`, color: isSel ? 'var(--accent)' : 'var(--text-muted)', fontSize: 12, fontWeight: 800, letterSpacing: '0.02em', transition: 'all 0.12s' }}>
                {name.slice(0, 2)}
              </button>
            );
          })}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 8, marginBottom: 20, background: countOk ? 'var(--phase-accum)11' : 'var(--danger)11', border: `1px solid ${countOk ? 'var(--phase-accum)44' : 'var(--danger)44'}` }}>
          <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{selected.length} of {requiredCount} days selected</span>
          {countOk
        ...
        <button onClick={() => { if (!countOk) return; setPendingDays([...selected]); setConfirmScope(true); }} disabled={!countOk} style={{ width: '100%', padding: '14px', borderRadius: 8, cursor: countOk ? 'pointer' : 'not-allowed', background: countOk ? 'var(--btn-primary-bg)' : 'var(--bg-deep)', border: `1px solid ${countOk ? 'var(--btn-primary-border)' : 'var(--border)'}`, color: countOk ? 'var(--btn-primary-text)' : 'var(--text-dim)', fontSize: 14, fontWeight: 800, letterSpacing: '0.06em', opacity: countOk ? 1 : 0.5 }}>
          SAVE SCHEDULE
        </button>
      </div>
    );
