...
function ScheduleTab({
  profile, activeDays, completedDays, activeWeek, currentWeek,
  calendarOffset, setCalendarOffset,
  expandedWeek, setExpandedWeek,
  showRestDay, setShowRestDay,
  showEditSchedule, setShowEditSchedule,
  noteViewer, setNoteViewer,
  skipVersion, setSkipVersion,
  goBack, goTo,
  onSelectDay, onSelectDayWeek, onOpenExtra, onOpenCardio,
  setCurrentWeek, onProfileUpdate,
  setAddWorkoutModal, setAddWorkoutStep, setAddWorkoutType, setAddWorkoutDayType,
}) {
  ...
  if (startDate && activeDays.length > 0) {
    ...
    for (let d = 0; d < 400 && sessionCount < totalSessions; d++) {
      ...
      if (getWorkoutDaysForWeek(profile, wkIdx).includes(cursor.getDay())) {
        sessionDateMap[cursor.toISOString().slice(0, 10)] = `${sessionCount % activeDays.length}:${wkIdx}`;
        sessionCount++;
      }
  ...
  const minOffset = startD
    ? (startD.getFullYear() - today.getFullYear()) * 12 + (startD.getMonth() - today.getMonth()) : -6;
  ...
  const maxOffset = endD
    ? (endD.getFullYear() - today.getFullYear()) * 12 + (endD.getMonth() - today.getMonth()) : 6;
  ...
  return (
    <div style={{ animation: 'tabFadeIn 0.15s ease-out' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 16px 12px', background: 'var(--bg-deep)', borderBottom: '1px solid var(--border)' }}>
        ...
        <button onClick={() => setShowEditSchedule(true)} style={{ padding: '6px 12px', borderRadius: 6, cursor: 'pointer', background: 'var(--accent)11', border: '1px solid var(--accent)44', color: 'var(--accent)', fontSize: 12, fontWeight: 700, letterSpacing: '0.04em' }}>⚙ Edit Schedule</button>
      </div>

      {/* Calendar */}
      <div style={{ padding: '12px 0 0' }}>
        <div style={{ width: '100%', background: 'var(--bg-card)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', padding: '14px 12px', boxShadow: 'var(--shadow-sm)' }}>
          {/* Calendar header */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 26, height: 26, borderRadius: 5, background: 'var(--phase-intens)15', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{scheduleIcon('var(--phase-intens)')}</div>
              <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <button onClick={() => canGoBack && setCalendarOffset(o => o - 1)} style={{ width: 32, height: 32, borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg-inset)', cursor: canGoBack ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', color: canGoBack ? 'var(--text-secondary)' : 'var(--text-dim)', fontSize: 16, fontWeight: 700, opacity: canGoBack ? 1 : 0.3 }}>‹</button>
              {calendarOffset !== 0 && <button onClick={() => setCalendarOffset(0)} style={{ padding: '4px 8px', borderRadius: 6, border: '1px solid var(--phase-intens)55', background: 'var(--phase-intens)11', cursor: 'pointer', fontSize: 12, fontWeight: 800, letterSpacing: '0.05em', color: 'var(--phase-intens)' }}>TODAY</button>}
              <button onClick={() => canGoForward && setCalendarOffset(o => o + 1)} style={{ width: 32, height: 32, borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg-inset)', cursor: canGoForward ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', color: canGoForward ? 'var(--text-secondary)' : 'var(--text-dim)', fontSize: 16, fontWeight: 700, opacity: canGoForward ? 1 : 0.3 }}>›</button>
            </div>
          </div>

          {/* DOW headers */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2, marginBottom: 4 }}>
            {DOW.map(d => <div key={d} style={{ textAlign: 'center', fontSize: 12, fontWeight: 700, letterSpacing: '0.03em', color: 'var(--text-secondary)', paddingBottom: 4 }}>{d}</div>)}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 2 }}>
            {cells.map((day, ci) => {
              ...
              let hasNotes = false, notesMeta = null;
              ...
              let bg = 'transparent', dateColor = 'var(--text-secondary)', borderColor = 'transparent';
              if (sessionKey && !isDone) { bg = isPast ? sessionPc + '28' : sessionPc + '30'; dateColor = isPast ? sessionPc + 'cc' : sessionPc; borderColor = isPast ? sessionPc + '55' : sessionPc + '88'; }
              if (isDone) { bg = sessionPc + '44'; dateColor = sessionPc; borderColor = sessionPc + '99'; }

              return (
                <div key={day} onClick={() => {
                  if (sessionKey) { const [dIdx, wIdx] = sessionKey.split(':').map(Number); onSelectDayWeek(dIdx, wIdx); }
                  else if (hasExtra) { onOpenExtra(dateStr); }
                  else if (hasCardio) { onOpenCardio(dateStr, cardioSession?.protocolId || null); }
                  else { setShowRestDay({ dateStr, isPast: isPast && dateStr !== todayStr }); }
                }} style={{ aspectRatio: '1', borderRadius: 5, background: bg, border: isToday ? `2px solid ${isDone ? sessionPc : sessionKey ? sessionPc : 'var(--phase-intens)'}` : `1px solid ${borderColor}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1, position: 'relative', cursor: 'pointer' }}>
                  <div style={{ fontSize: sessionKey ? 13 : 11, fontWeight: isToday || sessionKey ? 800 : 500, color: isToday ? (isDone ? sessionPc : sessionKey ? sessionPc : 'var(--phase-intens)') : dateColor, lineHeight: 1 }}>{day}</div>
                  {hasExtra && !hasNotes && <div style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--accent)', opacity: 0.9, position: 'absolute', top: 2, right: 2 }}/>}
                  {hasCardio && <div style={{ width: 5, height: 5, borderRadius: '50%', background: cardioDone ? '#D4983C' : TAG_ACCENT['CARDIO'], opacity: 0.9, position: 'absolute', bottom: 2, right: 2 }}/>}
                  {hasNotes && (
                    <div onClick={e => {
                      e.stopPropagation();
                      if (notesMeta.type === 'meso') {
                        ...
                        setNoteViewer({ type: 'meso', dayIdx: dIdx, weekIdx: wIdx, label: d ? `${d.label} — W${wIdx + 1}` : `Day ${dIdx + 1} W${wIdx + 1}`, exercises: d ? d.exercises : [], sessionNote: loadNotes(dIdx, wIdx), exNotes: loadExNotes(dIdx, wIdx) });
                      } else {
                        const extra = (() => { try { return JSON.parse(store.get(`foundry:extra:${notesMeta.dateStr}`) || 'null'); } catch { return null; } })();
                        setNoteViewer({ type: 'extra', dateStr: notesMeta.dateStr, label: extra ? extra.label : 'Extra Session', exercises: extra ? extra.exercises : [], sessionNote: store.get(`foundry:extra:notes:${notesMeta.dateStr}`) || '', exNotes: loadExtraExNotes(notesMeta.dateStr) });
                      }
                    }} style={{ position: 'absolute', top: 2, right: 2, fontSize: 12, lineHeight: 1, cursor: 'pointer', filter: 'drop-shadow(0 0 2px rgba(0,0,0,0.4))' }} title="View notes">📝</div>
                  )}
                </div>
              );
            })}
          ...
          <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {[['Accumulation', 'var(--phase-accum)'], ['Intensification', 'var(--phase-intens)'], ['Peak', 'var(--phase-peak)'], ['Deload', 'var(--phase-deload)']].map(([label, color]) => (
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 3 }}><div style={{ width: 10, height: 10, borderRadius: 3, background: color }}/><span style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>{label.slice(0, 3).toUpperCase()}</span></div>
              ))}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><div style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--accent)' }}/><div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Extra</div></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}><div style={{ width: 5, height: 5, borderRadius: '50%', background: '#D4983C' }}/><div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Cardio</div></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '8px 12px 0' }}>
        <button onClick={() => goTo('overview')} style={{ width: '100%', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, padding: '14px 16px', cursor: 'pointer', textAlign: 'left', boxShadow: 'var(--shadow-sm)', transition: 'border-color 0.15s, transform 0.12s', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 26, height: 26, borderRadius: 5, background: 'var(--phase-deload)18', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{overviewIcon('var(--phase-deload)')}</div>
            <div>
      ...
      <div style={{ padding: '0 16px' }}>
        {Array.from({ length: getMeso().weeks }, (_, w) => (
          <WeekSection key={w} weekIdx={w} currentWeek={activeWeek}
            onSelectDay={(dayIdx, weekIdx) => { setCurrentWeek(weekIdx); saveCurrentWeek(weekIdx); onSelectDay(dayIdx); }}
            completedDays={completedDays} isExpanded={expandedWeek === w}
      ...
      {/* Sheet overlays */}
      <RestDaySheet showRestDay={showRestDay} setShowRestDay={setShowRestDay} profile={profile} activeDays={activeDays} setAddWorkoutModal={setAddWorkoutModal} setAddWorkoutStep={setAddWorkoutStep} setAddWorkoutType={setAddWorkoutType} setAddWorkoutDayType={setAddWorkoutDayType} />
      <EditScheduleSheet showEditSchedule={showEditSchedule} setShowEditSchedule={setShowEditSchedule} profile={profile} currentWeek={currentWeek} onProfileUpdate={onProfileUpdate} />
