...
function lookupProtocolLabel(id: string): string {
  return CARDIO_WORKOUTS.find((w) => w.id === id)?.label ?? id;
}
...
interface CardioApplySheetProps {
  ...
  onApply: (nextSchedule: CardioScheduleSlot[], addedCount: number) => void;
  onClose: () => void;
}
...
export default function CardioApplySheet({
  ...
  const apply = () => {
    // Build next schedule: keep entries for *other* protocols on days we didn't touch,
    // remove entries for this protocol on days no longer selected, add entries for
    // newly-selected days (overwriting any conflicting other-protocol slots).
    const filtered = schedule.filter((s) => {
      if (selected.has(s.dayOfWeek)) return false; // we own this DOW now
      if (s.protocol === protocolId) return false; // remove any deselected entries of this protocol
      return true;
    });
