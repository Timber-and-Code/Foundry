...
/**
 * Custom rest-complete chime — matches the in-app WebAudio two-note (G5→C6)
 * cue so foreground and background cues are perceptually identical. Files
 * are bundled by Capacitor on cap sync. Generated by
 * `scripts/generate-rest-chime.py` — re-run if the chime ever changes.
 *
 *   iOS:     ios/App/App/rest-complete.wav    → ref by full filename
 *   Android: android/app/src/main/res/raw/rest_complete.wav → ref by
 *            resource name (no extension; lowercase + underscores only)
 */
function customChimeSound(): string | undefined {
  try {
    const platform = Capacitor.getPlatform();
    if (platform === 'ios') return 'rest-complete.wav';
    if (platform === 'android') return 'rest_complete';
  } catch {
      ...
      notifications: [
        {
          ...
          // Custom two-note chime matching the in-app WebAudio cue —
          // foreground and background ring the same. Falls back to the OS
          // default if the bundle is missing (e.g. dev build that hasn't
          // run cap sync since the file was generated).
          sound: customChimeSound(),
