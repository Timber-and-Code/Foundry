...
export function SetupPage({ onComplete }: SetupPageProps) {
  ...
  const handleSubmit = () => {
    ...
    if (cfg && !cfg.validDays.includes(form.workoutDays.length)) {
      setError(
        `${cfg.label} needs ${cfg.validDays.join(' or ')} training days — you have ${form.workoutDays.length} selected.`
      );
      return;
    }
    ...
    if (form.splitType === 'ppl')
      return n >= 6
        ...
        : n >= 5
          ? ['PUSH', 'PULL', 'LEGS']
          : ['PUSH', 'PULL', 'LEGS'];
    if (form.splitType === 'upper_lower') return n >= 4 ? ['UPPER', 'LOWER'] : ['UPPER', 'LOWER'];
    if (form.splitType === 'full_body') return ['FULL A', 'FULL B', 'FULL C'].slice(0, n);
    if (form.splitType === 'push_pull') return ['PUSH A', 'PULL A', 'PUSH B', 'PULL B'].slice(0, n);
    return [];
  };
  const tagColor = (tag) =>
    tag.startsWith('PUSH')
      ...
      >
              ...
              >
                <span
                  style={{
                    fontSize: 12,
                    fontWeight: 700,
                    color: isOn ? 'var(--text-secondary)' : 'var(--text-dim)',
        ...
        {mesoEnd && (
          ...
          >
            ...
            <span>
              End <b style={{ color: 'var(--text-primary)' }}>{mesoEnd}</b>
            </span>
          </div>
        )}
      </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
              ...
              <button
                onClick={() => {
                  ...
                  if (splitValidDays && !splitValidDays.includes(form.workoutDays.length)) {
                    setError(
                      `${SPLIT_CONFIG[form.splitType]?.label || form.splitType} needs ${splitValidDays.join(' or ')} training days — you have ${form.workoutDays.length} selected.`
                    );
                    return;
                  }
          ...
          {step === 2 &&
            ...
            (() => {
              ...
              const dayTemplates =
                splitDayTemplates[form.splitType]?.[numDays] ||
          ...
          {step === 2 &&
            ...
            (() => {
              ...
              const allDaysValid = dayTemplates.every(
                (_, i) => cardioDays.has(i) || (dayExercises[i] || []).length >= 3
              );
