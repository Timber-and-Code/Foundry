...
export function ProfileDrawer({ saved, onClose, onSave }) {
  ...
  const handleSave = () => {
    ...
    if (birthMonth && birthDay && birthYear) {
      ...
      const now = new Date();
      age = String(now.getFullYear() - bd.getFullYear() -
  ...
  const handleExport = () => {
    ...
    a.href = url; a.download = `foundry_backup_${new Date().toISOString().slice(0,10)}.json`;
  ...
  const inputStyle = {
    ...
    fontWeight:600, padding:"10px 12px", outline:"none", fontFamily:"inherit",
  };
  const selectStyle = {
    ...inputStyle, width:"auto", appearance:"none", WebkitAppearance:"none", cursor:"pointer",
  };
  const labelStyle = {
    fontSize:9, fontWeight:600, letterSpacing:"0.14em",
    color:"var(--text-dim)", marginBottom:3, textTransform:"uppercase",
  };
  const fieldRowStyle = {
  ...
  const fieldValueStyle = { fontSize:12, color:"var(--text-primary)", fontWeight:500 };

  const splitLabels = { ppl:"Push / Pull / Legs", upper_lower:"Upper / Lower", full_body:"Full Body" };
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          <span style={{fontSize:12, fontWeight:700, letterSpacing:"0.08em", color:"var(--text-secondary)"}}>PROFILE</span>
          <button onClick={onClose} style={{
            background:"var(--bg-inset)", border:"1px solid var(--border)",
            borderRadius:"50%", cursor:"pointer", width:28, height:28,
        ...
        <div style={{
          ...
          <img src={typeof FOUNDRY_PROFILE_IMG !== 'undefined' ? FOUNDRY_PROFILE_IMG : ''} alt=""
            style={{
              position:"absolute", inset:0, width:"100%", height:"100%",
              objectFit:"cover", objectPosition:"center 25%", opacity:0.45,
            }} />
          <div style={{
            position:"absolute", inset:0,
            background:"linear-gradient(to bottom, rgba(26,24,20,0.2) 0%, rgba(26,24,20,0.3) 35%, rgba(26,24,20,0.85) 75%, rgba(26,24,20,0.98) 100%)",
          }}/>
          <div style={{
            ...
            <div style={{
              ...
              fontFamily:"'Bebas Neue','Inter',sans-serif", fontSize:22, color:"#FBF7E4",
            ...
            <div>
              <div style={{fontSize:16, fontWeight:600, color:"#FBF7E4", textShadow:"0 1px 8px rgba(0,0,0,0.8)"}}>
                {form.name || "Athlete"}
              </div>
              <div style={{fontSize:11, color:"var(--phase-accum)", textShadow:"0 1px 4px rgba(0,0,0,0.8)"}}>
        ...
        <div style={{padding:"12px 18px", display:"flex", flexDirection:"column", gap:8, flex:1}}>
          ...
          <div style={{display:"flex", flexDirection:"column", gap:6}}>
            ...
            {totalWeeks && (
              <div style={fieldRowStyle}>
                ...
                <span style={fieldValueStyle}>Week {currentWeek} of {totalWeeks}</span>
              </div>
            )}
            {saved.weight && (
              <div style={fieldRowStyle}>
          ...
          {/* Edit profile toggle */}
          <button onClick={() => setShowEditFields(!showEditFields)} style={{
            ...
            <span style={{fontSize:10, color:"var(--text-muted)"}}>{showEditFields ? "▲" : "▼"}</span>
          </button>
          ...
          {showEditFields && (
            <div style={{display:"flex", flexDirection:"column", gap:12, paddingTop:4}}>
              ...
              <div>
                ...
                <input type="number" inputMode="decimal" placeholder="e.g. 185" value={form.weight}
                  ...
                  {[["m","Male"],["f","Female"]].map(([val, label]) => (
                    <button key={val} onClick={() => setForm(f => ({...f, gender: val}))}
                      style={{
                        ...
                        background: form.gender === val ? "rgba(var(--accent-rgb),0.12)" : "var(--bg-inset)",
          ...
          {/* Data section */}
          <div style={{height:1, background:"linear-gradient(90deg, transparent, rgba(232,101,26,0.2), transparent)", margin:"4px 0"}}/>
          <div style={{...labelStyle, marginTop:4}}>DATA</div>
          <div style={{display:"flex", flexDirection:"column", gap:6}}>
            <button onClick={handleExport} style={{
              ...fieldRowStyle, cursor:"pointer", border:"1px solid var(--border)",
            ...
            <button onClick={() => {
              ...
              <span style={{fontSize:13, color:"var(--danger)", fontWeight:500}}>Reset</span>
            </button>
          </div>
          ...
          {showFeedback && (
            <div style={{
              ...
              padding:20,
            }} onClick={(e) => { if (e.target === e.currentTarget && feedbackStatus !== "sending") { setShowFeedback(false); setFeedbackStatus(""); } }}>
              <div style={{
                background:"var(--bg-surface)", borderRadius:14, padding:20,
                width:"100%", maxWidth:360, border:"1px solid var(--border)",
              }}>
                <div style={{fontSize:14, fontWeight:700, color:"var(--text-primary)", marginBottom:4}}>Send Feedback</div>
                <div style={{fontSize:11, color:"var(--text-secondary)", marginBottom:12, textShadow:"0 1px 4px rgba(0,0,0,0.5)"}}>Bug reports, feature ideas, anything — it goes straight to the developer.</div>
                <textarea
                  value={feedbackMsg}
                ...
                <div style={{display:"flex", gap:8, marginTop:12}}>
                  <button onClick={() => { setShowFeedback(false); setFeedbackStatus(""); }} disabled={feedbackStatus === "sending"} style={{
                    flex:1, padding:"10px 0", borderRadius:8, fontSize:13, fontWeight:600,
                    background:"transparent", border:"1px solid var(--border)",
                    color:"var(--text-muted)", cursor:"pointer", fontFamily:"inherit",
                  }}>Cancel</button>
                  <button onClick={handleSendFeedback} disabled={!feedbackMsg.trim() || feedbackStatus === "sending" || feedbackStatus === "sent"} style={{
                    flex:1, padding:"10px 0", borderRadius:8, fontSize:13, fontWeight:600,
                    ...
                    {feedbackStatus === "sending" ? "Sending…" :
                     ...
                     feedbackStatus === "error" ? "Failed — Retry" : "Send"}
                  </button>
                </div>
                {feedbackStatus === "error" && (
                  <div style={{fontSize:11, color:"var(--danger)", textAlign:"center", marginTop:8}}>Something went wrong. Try again.</div>
                )}
              </div>
            </div>
          )}
          ...
          {/* Version */}
          <div style={{textAlign:"center", fontSize:10, color:"var(--text-dim)", marginTop:12, paddingBottom:20}}>
