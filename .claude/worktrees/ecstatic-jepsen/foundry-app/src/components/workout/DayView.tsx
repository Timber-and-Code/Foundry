...
function DayView({
  ...
  const weekDataRef = React.useRef(weekData);
  React.useEffect(() => { weekDataRef.current = weekData; }, [weekData]);
  ...
  const handleUpdateSet = useCallback(
    (exIdx: number, setIdx: number, field: string, value: string | number | boolean) => {
      ...
      // Auto-advance: if user confirmed the last set of an exercise, expand the next one
      if (field === 'confirmed' && value === true) {
        const ex = exercises[exIdx];
        const numSets = Number(ex?.sets) || 0;
        const currentExData = weekDataRef.current[exIdx] || {};
        let allConfirmed = true;
        for (let s = 0; s < numSets; s++) {
          if (s === setIdx) continue; // this set is being confirmed now
          const sd = (currentExData[s] || {}) as Record<string, unknown>;
          if (!sd.confirmed) { allConfirmed = false; break; }
        }
        if (allConfirmed && exIdx + 1 < exercises.length) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {(isDone || isLocked) && <div style={{ width: 48 }} />}
      </div>
      ...
      {showEndEarlyConfirm && (
        ...
        >
          ...
          >
            <div
              id="end-early-title"
              style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 8 }}
            >
              End workout early?
            </div>
