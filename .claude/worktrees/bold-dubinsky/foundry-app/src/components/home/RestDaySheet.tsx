...
function RestDaySheet({
    ...
    for (let d = 0; d < 400 && sc3 < total3; d++) {
      ...
      if (getWorkoutDaysForWeek(profile, wkIdx3).includes(cursor3.getDay())) {
        if (cursor3 >= fromDate) {
          ...
          if (nd)
            nextSessionLabel = `${nd.label} · W${wkIdx3 + 1} · ${nd.exercises?.length || 0} exercises`;
          break;
        }
        sc3++;
      }
  ...
  const recoveryItems = [
    ...
    {
      title: 'Protein',
      body: 'Hit your target today even without training. Muscle protein synthesis continues in recovery — shortchanging protein now blunts your gains from yesterday.',
    },
    {
      title: 'Walk',
