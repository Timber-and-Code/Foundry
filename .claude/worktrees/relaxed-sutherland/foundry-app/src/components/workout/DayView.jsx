...
function DayView({
  ...
  const _todayReadinessKey = (() => {
    const d = new Date();
    return `foundry:readiness:${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
  })();
  const [dvReadiness, setDvReadiness] = useState(() => {
    try {
      return JSON.parse(store.get(_todayReadinessKey) || 'null');
    } catch {
      return null;
    }
  });
  const dvReadinessFilled =
    dvReadiness && dvReadiness.sleep && dvReadiness.soreness && dvReadiness.energy;
  ...
  const stallingData = React.useMemo(() => {
    if (isDone || isLocked || getWeekPhase()[weekIdx] === 'Deload')
  ...
  const [stallCardDismissed, setStallCardDismissed] = useState(
    () => !!store.get(`foundry:sessionStart:d${dayIdx}:w${weekIdx}`)
  );
  ...
  const handleSetLogged = React.useCallback(
    (restStr, exName, setIdx, isLastSet = false) => {
  ...
  const handleAddSet = () => {
    ...
    setExercises((prev) => prev.map((e, i) => (i === exIdx ? { ...e, sets: e.sets + 1 } : e)));
