...
import { ExerciseDetailModal, TAG_COLORS } from './ExerciseDetailModal';

const ALL_TAGS = ['ALL', 'PUSH', 'PULL', 'LEGS'];
const ALL_EQUIPS = ['ALL', 'barbell', 'dumbbell', 'cable', 'machine', 'bodyweight', 'kettlebell', 'band'];
const ALL_MUSCLES = [
  'ALL', 'Chest', 'Back', 'Lats', 'Shoulders', 'Traps', 'Biceps', 'Triceps',
  'Forearms', 'Core', 'Abs', 'Obliques', 'Quads', 'Hamstrings', 'Glutes', 'Calves', 'LowerBack',
];
const PATTERN_MAP: Record<string, string> = {
  ALL: 'All', push: 'Press', pull: 'Row/Pull', squat: 'Squat',
  hinge: 'Hinge', carry: 'Carry', isolation: 'Isolation',
};
const ALL_PATTERNS = ['ALL', 'push', 'pull', 'squat', 'hinge', 'carry', 'isolation'];

const equipLabels: Record<string, string> = {
  ALL: 'All', barbell: 'Barbell', dumbbell: 'Dumbbell', cable: 'Cable',
  machine: 'Machine', bodyweight: 'Bodyweight', kettlebell: 'Kettlebell', band: 'Band',
};
const muscleLabels: Record<string, string> = {
  ALL: 'All Muscles', Chest: 'Chest', Back: 'Back', Lats: 'Lats', Shoulders: 'Shoulders',
  Traps: 'Traps', Biceps: 'Biceps', Triceps: 'Triceps', Forearms: 'Forearms', Core: 'Core',
  Abs: 'Abs', Obliques: 'Obliques', Quads: 'Quads', Hamstrings: 'Hamstrings',
  Glutes: 'Glutes', Calves: 'Calves', LowerBack: 'Lower Back',
};
...
function FilterRow({ label, options, value, labelMap, onChange }: FilterRowProps) {
  return (
...
export function ExerciseBrowser({ onBack }: ExerciseBrowserProps) {
  const [libFilter, setLibFilter] = useState({
    tag: 'ALL', equip: 'ALL', muscle: 'ALL', pattern: 'ALL', search: '',
  });
  const [showFilterPanel, setShowFilterPanel] = useState(false);
  const [showHowTo, setShowHowTo] = useState<(typeof EXERCISE_DB)[number] | null>(null);

  const activeFilterCount = [libFilter.tag, libFilter.equip, libFilter.muscle, libFilter.pattern]
    ...
    <div style={{ animation: 'tabFadeIn 0.15s ease-out', paddingBottom: 90 }}>
      ...
      {!showFilterPanel && activeFilterCount > 0 && (
        ...
        >
          ...
          <button
            onClick={() =>
              setLibFilter((f) => ({ ...f, tag: 'ALL', equip: 'ALL', muscle: 'ALL', pattern: 'ALL' }))
            }
            style={{
              padding: '3px 10px',
              borderRadius: tokens.radius.round,
