...
const SwapModal = ({ exercise: _exercise, dayTag: _dayTag, profile: _profile, onSwap: _onSwap, onClose }: { exercise?: Exercise; dayTag?: string; profile?: Profile; onSwap?: (ex: Exercise) => void; onClose: () => void }) => (
  <div
    style={{
      position: 'fixed',
      inset: 0,
...
const AddExerciseModal = ({ dayTag: _dayTag, profile: _profile, currentExerciseIds: _currentExerciseIds, onAdd: _onAdd, onClose }: { dayTag?: string; profile?: Profile; currentExerciseIds?: (string | number)[]; onAdd: (ex: Exercise) => void; onClose: () => void }) => (
  <div
    style={{
      position: 'fixed',
      inset: 0,
...
const WorkoutCompleteModal = ({ dayLabel, dayTag: _dayTag, gender: _gender, stats, weekIdx: _weekIdx, onDone, onClose }: { dayLabel: string; dayTag?: string; gender?: string; stats?: { totalSets: number; duration?: string | null }; weekIdx?: number; onDone?: () => void; onClose: () => void }) => (
  <div
    style={{
      position: 'fixed',
      inset: 0,
...
function ExtraDayView({ dateStr, onBack, profile, onProfileUpdate, activeDays }: ExtraDayViewProps) {
  ...
  const getCarryoverWeight = (exId: string | number) => {
    if (!activeDays) return '';
    const weeks = getMeso().weeks;
    for (let w = weeks; w >= 0; w--) {
      for (let d = 0; d < activeDays.length; d++) {
        const idx = activeDays[d].exercises.findIndex((e: Exercise) => e.id === exId);
        if (idx < 0) continue;
        try {
  ...
  // ── Handlers ─────────────────────────────────────────────────────────────────
  const handleUpdateSet = (exIdx: number, setIdx: number, field: string, value: unknown) => {
  ...
  const handleWeightAutoFill = (exIdx: number, weight: number | string) => {
