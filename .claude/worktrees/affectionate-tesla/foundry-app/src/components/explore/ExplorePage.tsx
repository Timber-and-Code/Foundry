...
import type { Profile, Exercise } from '../../types';
...
interface SampleProgramDay {
  label?: string;
  name?: string;
  tag?: string;
  exercises: (string | { name?: string; sets?: number; repRange?: string; progression?: string; warmup?: string | null })[];
}
...
interface SampleProgram {
  ...
  splitType?: string;
  description?: string;
  days: SampleProgramDay[];
}

// Build AI days array from a sample program's day definitions
const buildAiDaysFromSample = (prog: SampleProgram) => {
  if (!prog || !prog.days) return [];
  return prog.days.map((day: SampleProgramDay) => ({
    label: day.label || day.name || 'Day',
    tag: day.tag || 'PUSH',
    exercises: (day.exercises || []).map((ex) => ({
      name: typeof ex === 'string' ? ex : (ex.name || ''),
      sets: typeof ex === 'string' ? 3 : (ex.sets || 3),
      repRange: typeof ex === 'string' ? '8-12' : (ex.repRange || '8-12'),
      progression: typeof ex === 'string' ? 'standard' : (ex.progression || 'standard'),
      warmup: typeof ex === 'string' ? null : (ex.warmup || null),
    })),
  }));
};

// Modal for starting a sample program
const StartSampleProgramModal = ({ prog, hasActiveMeso, onConfirm, onCancel }: { prog: SampleProgram; hasActiveMeso: boolean; onConfirm: (startDate: string) => void; onCancel: () => void }) => {
...
interface ExplorePageProps {
  profile: Profile;
  onStartProgram?: (program: SampleProgram) => void;
}
...
function ExplorePage({ profile, onStartProgram }: ExplorePageProps) {
  ...
  const HowToModal = ({ ex, onClose }: { ex: Exercise; onClose: () => void }) => (
    <div
      style={{
        position: 'fixed',
        inset: 0,
  ...
  if (section === 'library') {
    ...
    const FilterRow = ({ label, options, value, labelMap, onChange }: { label: string; options: string[]; value: string; labelMap: Record<string, string>; onChange: (v: string) => void }) => (
