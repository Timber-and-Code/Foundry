...
// ── Helpers ──────────────────────────────────────────────────────────────────
function sessionTagToCategory(sessionTag: string, exTag: string) {
  ...
  activeDays.forEach((day: TrainingDay, dayIdx: number) => {
    for (let w = 0; w <= getMeso().weeks; w++) {
      ...
      day.exercises.forEach((ex: Exercise, exIdx: number) => {
        ...
        const filledSets = Object.values(exData).filter((s) => s && (s as Record<string, unknown>).reps && (s as Record<string, unknown>).reps !== '').length;
  ...
  return (
    <div>
      ...
      <div style={{ padding: '0 16px 24px' }}>
        ...
        {/* e1RM card */}
        {(() => {
          const anchors: { name: string; tag: string; e1rm: number; allTimeBest: number; isPR: boolean; load75: number; load85: number; isStalling: boolean }[] = [];
          activeDays.forEach((day: TrainingDay, dayIdx: number) => {
            day.exercises.forEach((ex: Exercise, exIdx: number) => {
              if (!ex.anchor) return;
        ...
        {activeDays.map((day, dayIdx) => {
          ...
          const dayLifts = day.exercises.map((ex: Exercise, exIdx: number) => {
            ...
            for (let w = currentWeek; w >= 0; w--) {
              const wd = loadDayWeek(dayIdx, w);
              const exData = wd[exIdx] || {};
              const sw = Object.values(exData).find((sv) => sv && (sv as Record<string, unknown>).weight && (sv as Record<string, unknown>).weight !== '');
              if (sw) {
                weight = (sw as { weight: string | number }).weight;
                break;
              }
            }
            return {
