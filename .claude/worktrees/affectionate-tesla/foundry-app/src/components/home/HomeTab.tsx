...
import type { Profile, TrainingDay, Exercise, CardioScheduleSlot, ReadinessEntry } from '../../types';
import {
  TAG_ACCENT,
  getMeso,
  DAILY_MOBILITY,
...
function RestStateCard({
  ...
  setShowRecoveryOpen: React.Dispatch<React.SetStateAction<boolean>>;
  showNextSession: boolean;
  setShowNextSession: React.Dispatch<React.SetStateAction<boolean>>;
  activeWeek: number;
  goBack: () => void;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      >
        ...
        <div style={{ padding: '0' }}>
          <button
            onClick={() => setShowRecoveryOpen((p) => !p)}
            style={{
              width: '100%',
...
interface HomeTabProps {
  ...
  showNextSession: boolean;
  setShowNextSession: (v: boolean) => void;
  showMorningMobility: boolean;
  setShowMorningMobility: (v: boolean) => void;
  goTo: (v: string) => void;
  goBack: () => void;
  onSelectDay: (v: number) => void;
  ...
  onOpenCardio: (dateStr: string, protocol: string) => void;
...
function HomeTab({
  ...
  const showDayAccent = showDay ? (TAG_ACCENT as Record<string, string>)[showDay.tag ?? ''] : nextDayAccent;
  ...
  const lastWeekData: Record<string, Record<string, { weight?: string | number; reps?: string | number }>> = (() => {
          ...
          <div style={{ padding: '6px 0 2px' }}>
            {preview.map((ex: Exercise, ei: number) => {
              ...
              const prevSets = prevData
                ? (Object.values(prevData) as { weight?: string | number; reps?: string | number }[]).filter((s) => s && s.weight && parseFloat(String(s.weight)) > 0)
                : [];
