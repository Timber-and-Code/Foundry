...
import type { WorkoutSet, Exercise, TrainingDay } from '../types';
...
export function useMesoState({ setView, setOnboarded }: UseMesoStateParams) {
  ...
  const activeDays = useMemo(() => {
    ...
    return days.map((day: TrainingDay, dayIdx: number) => {
      const extraIds = (added as Record<string, (string | number)[]>)[dayIdx] || [];
      if (extraIds.length === 0) return day;
      const extraExs = extraIds
        .map((id: string | number) => EXERCISE_DB.find((e: Exercise) => e.id === id))
        .filter(Boolean)
        .map((e: Exercise) => ({
          id: e.id,
          name: e.name,
